...
/* KSUN glue: upstream KSUN has no susfs integration, so backport the two
 * symbols the susfs-1.4.2-for-4.14 code expects from the KSU tree. */
...
static void susfs_ksu_try_umount(const char *mnt, bool check_mnt, int flags)
{
	struct path path;
...
{
	const struct cred *saved = override_creds(prepare_kernel_cred(NULL));

	(void)uid;
	susfs_ksu_try_umount(mnt, check_mnt, flags);
	revert_creds(saved);
}
...
/* KSUN has no sus_su hooks; provide no-op stubs so susfs links. */
